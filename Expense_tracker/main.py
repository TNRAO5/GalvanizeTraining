### Expenses tracker by Narayana Tangudu, Satya Medisetti
...
import requests                         # importing the requests for API
...
def dollarToINR():
    ...
    url = f'https://www.alphavantage.co/query?function=CURRENCY_EXCHANGE_RATE&from_currency=USD&to_currency=INR&apikey=BATGXSHMU6JSPF8E'
    ...
    # returns exchange rate in popup window 
        mb.showinfo('USD to INR', f'Today USD to INR rate is: {Exchange_Rate}') 
        
    else:
        print('Error fetching Exchange data')
...
if __name__ == "__main__":  
    ...
    # Mote: Please enter all the fields  
    subheadingLabel2 = Label(  
        frameL1,  
        text = "NOTE: PLEASE ENTER ALL THE FIELDS",  
        font = ("Bahnschrift Condensed", "10"),  
        width = 10,  
        bg = "#F5DEB3",  
        fg = "#000000"  
        )  
    ...
    subheadingLabel2.pack(fill = "both")  
    ...
    modeField = OptionMenu(  
        frameL2,  
        modeOfPayment,  
        *['Cash', 'Cheque', 'Credit Card', 'Debit Card', 'Paytm', 'Google Pay', 'PhonePe']  
        )  
    ...
    viewAllButton.grid(row = 1, column = 2, sticky = W, padx = 10, pady = 10)  
    ...
    # adding different columns to the table with width
    ...
    data_table.column('#4', width = 350, stretch = NO)  
    data_table.column('#5', width = 100, stretch = NO)  
